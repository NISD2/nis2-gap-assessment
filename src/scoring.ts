...
function getMaturityLevel(percentage: number): MaturityKey {
  for (const level of MATURITY_LEVELS) {
    if (percentage >= level.min && percentage <= level.max) return level.key;
  }
  return "critical";
}
...
export function computeDomainScores(
  ...
  return domainIds.map((domainId) => {
    ...
    const scored = domainQuestions.filter(
      (q) => answers[q.id] !== undefined && answers[q.id] !== ANSWER.NA,
    );
